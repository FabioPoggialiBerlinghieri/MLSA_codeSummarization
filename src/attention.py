...
class Attention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, mask=None):
        ...
        # query : B x 1 x Input
        if query.dim() != 3:
            raise ValueError(f"Expected query to be a 3D tensor (B, 1, Input), but received a {query.dim()}D tensor.")
        if query.shape[1] != 1:
            raise ValueError(
                f"Expected query sequence length to be 1 (B, 1, Input), but received {query.shape[1]} at dimension 1.")
        ...
        if mask is not None:
            if mask.dim() != 3:
                raise ValueError(f"Expected mask to be a 3D tensor (B, 1, L), but received a {mask.dim()}D tensor.")
            if mask.shape[1] != 1:
                raise ValueError(
                    f"Expected mask sequence length to be 1 (B, 1, L), but received {mask.shape[1]} at dimension 1.")
        ...
        # score: B x 1 x D_Model * B x D_Model x L = B x 1 x L
